...
#ifndef __LcmGui_Imu_hpp__
#define __LcmGui_Imu_hpp__
...
int Imu::_encodeNoHash(void *buf, int offset, int maxlen) const
{
    ...
    tlen = __double_encode_array(buf, offset + pos, maxlen - pos, &this->angular_rate[0], 3);
    if(tlen < 0) return tlen; else pos += tlen;
...
int Imu::_decodeNoHash(const void *buf, int offset, int maxlen)
{
    ...
    tlen = __double_decode_array(buf, offset + pos, maxlen - pos, &this->angular_rate[0], 3);
    if(tlen < 0) return tlen; else pos += tlen;
...
uint64_t Imu::_computeHash(const __lcm_hash_ptr *p)
{
    ...
    uint64_t hash = 0xccf297d55d257707LL +
